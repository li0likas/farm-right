...
export default () => {
    ...
    const buy = (itemId, itemPrice, points) => {
        const token = localStorage.getItem('accessToken');
  ...
  return isLoggedIn() ? (
    ...
    <div className="container bg-white pt-12">
        ...
        <div className="mt-4 mb-4 mr-7 text-right">
            <p>My points: {points.userPoints}</p>
        </div>
                ...
                {userItems.map((item) => (
                        <div>
                            <div className="flex">
                                ...
                                <p className="text-[#61E9B1] pl-2"> -{item.price} points</p>
                            </div>
                            <p className="pl-6">({item.boughtAt ? item.boughtAt.split('T')[0] : null})</p>
                        </div>
                ))}
